...
def mvn_historical_drawdowns(Code, Price_Type, Period_Start='Inception', Period_End='Latest', Rank=1):
    ...
    # work_df['Recovery_Days'] = np.where(pd.isnull(work_df),
    #                                     pd.to_datetime(end_date) - work_df.index,
    #                                     work_df['Next_PP_index'] - work_df.index)
    ...
    drawdown_performance = np.round(results_df['Drawdown'].values, 6)
